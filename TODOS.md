...
- Extensions system: [`SPECs/extensions-system-spec.md`](SPECs/extensions-system-spec.md) — **high risk, largest bet in the backlog**. Guest API, QuickJS host, capability broker, Rust permission gate, host-rendered UI primitives, and both core extensions (`semantic-index`, `ai-chat`) are built and test-verified end to end. AI is deliberately **not** a host capability: the host exposes generic `process.*` primitives behind a new `unsafe` permission tier, and `ai-chat` ships its own Agent Client Protocol client written in TypeScript inside the sandbox, so users can point it at any locally installed agent (Copilot, Claude Code, Codex, Gemini). Distribution is now built and reachable: install and update by `owner/repo` from GitHub releases, two-phase resolve/commit so the host installs only the reviewed bytes, permission-diff re-consent that stays quiet for same-permission updates, keychain-stored PAT for private repos (read by Rust, never exposed to the WebView), atomic install with rollback, and uninstall — all surfaced in Preferences → Extensions. Semantic search now runs on real embeddings: potion-base-8M via `model2vec-rs` (pure Rust, no ONNX runtime, so no dylib to notarize), downloaded on request from Preferences rather than bundled or fetched on launch, with the bag-of-words fallback reported in the UI instead of degrading silently. The official registry (`registry/extensions.json`) and on-demand update checks are built: the registry is a lookup table that grants nothing, and updates funnel through the same resolve/commit consent path as any other install, with per-extension check failures reported rather than folded into "up to date". Runtime-tier permission prompts are built: `workspace.write` and unrestricted network are refused by Rust with a distinct `needs-approval` signal, the host asks once per extension+permission and retries exactly once, and allow-once is held in memory only so it cannot be promoted to always by a disk write. Uninstall forgets every decision. Extension panels are now actually reachable - a dock, command-palette entries appended after the built-ins, and lazy VM creation on first open. Persisted decisions are now listed per extension in Preferences with an "Ask again" control that clears the session grant as well as the stored one, so revoking takes effect immediately; revoking deliberately skips the manifest validation that granting requires, since it can only ever reduce access. WASM under the app's real CSP in WKWebView is now verified rather than assumed: a release build serves the policy as an HTTP header on `tauri://`, and the VM self test reports its result out of the WebView to stderr, where it reads `engine=wasm ... 1 + 1 = 2`. This also corrected a false verification: a debug build follows `devUrl`, so every earlier "the app launches clean" check was pointing at a dev server that was not running. Scheduled update checks are now built as an opt-in preference rather than an unconditional poller: a check spends the user's network and, if one is stored, their GitHub token, so it is off by default for the same reason the embedding model is not downloaded on launch. The 24h interval is enforced in Rust so a UI remount cannot turn it into a per-launch poller, and a backwards clock counts as due rather than silently disabling checks. The Preferences surfaces were also finally _looked at_ rather than only type-checked, which found dead CSS: `.is-primary` was applied to three consent buttons and styled nothing. It is now styled for the install flows and deliberately removed from the runtime permission prompt, where highlighting the broadest option would be the dialog answering its own question. That look-at-it pass continued into the extension UI library itself and found it was 40% built: 25 of 42 declared components had no host renderer and silently rendered nothing, while type checks, the unit suite, and the release build were all green. All 42 now render, and the structural gap is closed by a drift test that diffs the guest component library against the host renderer registry in both directions - components are tagged with a symbol so the walker enumerates exactly what is renderable rather than guessing from names. Prop hoisting was also switched from a hand-maintained name list to value-based detection, which kills the same class of drift permanently and removes a latent collision where `target` means a subtree on `Action.Push` but a URL on `Detail.Metadata.Link`. Rendering it in a browser then found five more silent faults invisible to every check: a 0px-wide search input, a transparent action popover, six CSS tokens referenced but defined nowhere, a mis-stacked section header, and an icon layer no test covered. Dead tokens are now a test (parse the stylesheet, scan the components, diff), status hues are defined as fixed values rather than accent-derived because the accent is the affirmative colour and a destructive action must not borrow it, and `Form.FilePicker` was removed rather than implemented since a native picker hands an extension absolute paths outside the workspace that no capability covers. The two core extensions now ship with the app rather than being copied in by hand: they are bundled at build time into `src-tauri/core-extensions/` and seeded into the user's extensions folder on first launch, version-aware so an app upgrade replaces an older built-in and a same-or-newer install is a no-op, and written without an `install.json` so the updater never treats a built-in as a GitHub release. The one accepted limitation is that removing a built-in brings it back on next launch, because `ai-chat` depends on `semantic-index`'s search service and both are treated as part of the app for now.
